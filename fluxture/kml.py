...
class ScaledKMLGraphNode(KMLGraphNode):
    ...
    def to_placemark(self, style: Optional[Style] = None) -> Placemark:
        scaled_style = Style(KML_NS, styles=[IconStyle(KML_NS, id="ip", scale=self.scale)])
...
def to_kml(
    ...
    for geolocation in tqdm(locations, leave=False, desc="Generating KML", unit=" locations"):
        ...
        for neighbor in geolocation.neighbors():
            ...
            p.geometry = Geometry(
                geometry=LineString([
                    ...
                    enumerate(geolocation.path_to(neighbor, intermediate_points=num_segments-2))
                ]),
                tessellate=False,
                extrude=False,
...
def calculate_rank(loc: KMLGraphNode, pr: OrderedDict[CrawledNode, float], db: CrawlDatabase) -> float:
        ...
        with CrawlDatabase(args.CRAWL_DB_FILE) as db:
            ...
            if args.group_by != "ip":
                ...
                collections: Dict[str, KMLGraphNodeCollection] = {
                    group: KMLGraphNodeCollection(group, subnodes=subnodes) for group, subnodes in groups.items()
                }
                for group, c in collections.items():
                    all_neighbors = set()
                    for member in groups[group]:
                        all_neighbors |= {grouper(neighbor) for neighbor in member.neighbors()}
                    all_neighbors -= {group}
                    c.set_neighbors((collections[neighbor_group] for neighbor_group in all_neighbors))
