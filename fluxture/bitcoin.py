...
from typing import (AsyncIterator, Dict, FrozenSet, Generic, KeysView, List,
                    Optional, Set, Tuple, Type, TypeVar, Union)

import fluxture.structures

from . import serialization
from .blockchain import (Blockchain, BlockchainError, Miner, Node, Version,
                         get_public_ip)
from .messaging import BinaryMessage
from .serialization import ByteOrder, P, UnpackError
from .shodan import SearchQuery, ShodanResult, get_api
...
NODE_QUERY = SearchQuery.register(
    name="BitcoinNode", query='port:8333 product:"/Satoshi:*/"'
)
...
class BitcoinMessageHeader(BinaryMessage):
    ...
    def decoded_command(self) -> str:
        ...
        if any(c != "\0" for c in decoded[first_null_byte:]):
            raise ValueError(
                f"Command name {self.command!r} includes bytes after the null terminator!"
            )
    ...
    def __repr__(self):
        return (
            f"{self.__class__.__name__}(magic={self.magic!r}, command={self.decoded_command!r}, "
...
class BitcoinMessage(BinaryMessage, ABC):
    ...
    def __init_subclass__(cls, **kwargs):
        if cls.command is None:
            raise TypeError(
                f"{cls.__name__} extends BitcoinMessage but does not speficy a command string!"
            )
        elif cls.command in MESSAGES_BY_COMMAND:
            raise TypeError(
                f"The command {cls.command} is already registered to message class "
    ...
    def deserialize_partial(
        cls, data: bytes, header: Optional[BitcoinMessageHeader] = None
    ) -> Tuple["BitcoinMessage", bytes]:
        if header is None:
            header, payload = BitcoinMessageHeader.unpack_partial(
                data, byte_order=BitcoinMessageHeader.byte_order
            )
        else:
            payload = data
        if header.magic != BITCOIN_MAINNET_MAGIC:
            raise ValueError(
                f"Message header magic was {header.magic}, but expected {BITCOIN_MAINNET_MAGIC!r} "
                "for Bitcoin mainnet!"
            )
        elif header.length < len(payload):
            raise ValueError(
                f"Invalid payload length of {len(payload)}; expected at least {header.length} bytes"
            )
        elif header.decoded_command not in MESSAGES_BY_COMMAND:
            raise NotImplementedError(
                f'TODO: Implement Bitcoin command "{header.command}"'
            )
        ...
        if header.checksum != expected_checksum:
            raise ValueError(
                f"Invalid message checksum; got {header.checksum!r} but expected {expected_checksum!r}"
            )
        return (
            MESSAGES_BY_COMMAND[decoded_command].unpack(
                payload, MESSAGES_BY_COMMAND[decoded_command].byte_order
            ),
            remainder,
        )
    ...
    async def next_message(
        ...
        try:
            ...
            raise ValueError(
                f"Expected {header.length} bytes when reading the message with header {header!r}"
            )
...
class VarInt(int, serialization.AbstractPackable):
    ...
    def pack(
        self, byte_order: serialization.ByteOrder = serialization.ByteOrder.LITTLE
    ) -> bytes:
        value = int(self)
        if value < 0xFD:
            return bytes([value])
            ...
            raise ValueError(
                f"Value {value} must be less than {serialization.UInt64.MAX_VALUE}"
            )

    @classmethod
    def unpack_partial(
        cls: Type[P], data: bytes, byte_order: ByteOrder = ByteOrder.LITTLE
    ) -> Tuple[P, bytes]:
        if data[0] < 0xFD:
            return cls(data[0]), data[1:]
        elif data[0] == 0xFD:
            return (
                cls(serialization.UInt16.unpack(data[1:3], byte_order=byte_order)),
                data[3:],
            )
        elif data[0] == 0xFE:
            return (
                cls(serialization.UInt32.unpack(data[1:5], byte_order=byte_order)),
                data[5:],
            )
        elif data[0] == 0xFF:
            return (
                cls(serialization.UInt64.unpack(data[1:9], byte_order=byte_order)),
                data[9:],
            )
        else:
...
class VarStr(bytes, serialization.AbstractPackable):
    ...
    def unpack_partial(
        ...
        if len(remainder) < length:
            raise UnpackError(
                f"Expected a byte sequence of length {length} but instead got {remainder!r}"
            )
    ...
    async def read(
        ...
        if len(string) < length:
            raise UnpackError(
                f"Expected a byte sequence of length {length} but instead got {string!r}"
            )
        return string
...
class NetIP(fluxture.structures.PackableStruct):
    ...
    def __init__(self, time: Optional[int] = None, addr: Optional[NetAddr] = None):
        if time is None:
...
class AbstractList(list, Generic[P], List[P], serialization.AbstractPackable, ABC):
    ...
    def pack(self, byte_order: ByteOrder = ByteOrder.NETWORK) -> bytes:
        return VarInt(len(self)).pack(byte_order) + b"".join(
            element.pack(byte_order) for element in self
        )
    ...
    def unpack_partial(
        ...
        return (
            cls(
                cls.ELEMENT_TYPE.unpack(bytes(data), byte_order=byte_order)
    ...
    async def read(
        ...
        return cls(
            cls.ELEMENT_TYPE.read(reader, byte_order=byte_order) for _ in range(length)
        )
...
class BitcoinNode(Node):
    ...
    async def connect(self):
        ...
        if not self.connected:
            raise BitcoinError(
                f"Did not receive a Verack message from client {self.address}:{self.port}"
            )
        ...
        async with self:
            ...
            async for msg in self.run():
                if isinstance(msg, AddrMessage):
                    return msg
        raise BitcoinError(
            f"Node {self.address}:{self.port} closed the connection before replying to our "
            "GetAddr message"
        )
        ...
        async with self:
            async for _ in self.run():
                if self.version is not None:
                    return self.version
        raise BitcoinError(
            f"Node {self.address}:{self.port} closed the connection before sending us a VersionMessage"
        )
...
async def collect_addresses(url: str, port: int = 8333) -> Tuple[BitcoinNode, ...]:
    return tuple(
        ...
        for addr in await asyncio.get_running_loop().getaddrinfo(
            url, port, proto=socket.IPPROTO_TCP
        )
    )
...
async def collect_defaults(
    *args: Union[Tuple[str], Tuple[str, int]], use_shodan: bool = True
    ...
    if use_shodan:
        shodan_iterator: Optional[AsyncIterator[ShodanResult]] = NODE_QUERY.run_async(
            get_api()
        ).__aiter__()
...
class Bitcoin(Blockchain[BitcoinNode]):
    ...
    async def is_miner(self, node: BitcoinNode) -> Miner:
        ...
        async with self._miner_query_lock:
            is_miner = (
                self._miners is not None
                and self._finished_miners_query
                and node.address in self._miners
            )
